...
class Navbar extends Component {
  ...
  getLink = ({ show, linkIndex, onClick, icon, label, style }) => {
    return (
      show && (
        <li
          className={this.linkStyle(linkIndex)}
          onClick={onClick}
          style={style}
        >
  ...
  render() {
    ...
    return (
      <ul className={Styles.navbar}>
        <li onClick={() => this.onHandleClick("/", 0)}>
          <h2 className={Styles.listHeader}>Blogger</h2>
        </li>
        {this.getLink({
          show: !!user.userId,
          linkIndex: 1,
...
export default connect(mapStateToProps, { logout, updateNavigation })(
  withRouter(Navbar)
);
